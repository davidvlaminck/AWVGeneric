...
def verleng_bestekkoppelingen(client: EMInfraClient,
    ...
    for counter, df_row in df.iterrows():
        ...
        if (matching_bestekkoppeling and
                ...
                datetime(year=2026, month=1, day=6).date()):
...
if __name__ == '__main__':
    ...
    df_assets = pd.read_csv(input_filepath, delimiter=',', header=0, usecols=["uuid", "naampad"])
    bestekref_mdm_19a03 = eminfra_client.bestek_service.get_bestekref(eDelta_besteknummer=EDELTA_BESTEKNUMMER)
    verleng_bestekkoppelingen(client=eminfra_client, df=df_assets, bestekref=bestekref_mdm_19a03)
