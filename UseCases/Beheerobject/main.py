...
if __name__ == '__main__':
    ...
    ##########################################
    beheerobjecttypes = eminfra_client.beheerobject.get_beheerobjecttypes()
    ...
    ##########################################
    response = eminfra_client.beheerobject.create_beheerobject(naam='dummyTest', beheerobjecttype=beheerobjecttype_installatie)
    ...
    myDummyBeheerobject = eminfra_client.beheerobject.get_beheerobject(beheerobject_uuid=myDummyBeheerobject_uuid)

    ##########################################
    # Oplijsten assets in de boomstructuur van een beheerobject
    ##########################################
    # WW0402 uuid: 0014b499-2bdb-448a-b3a1-fd20595701dc
    myAsset = eminfra_client.assets.get_asset(asset_uuid='0014b499-2bdb-448a-b3a1-fd20595701dc')
    generator_beheerobjecten = eminfra_client.assets.search_child_assets(asset=myAsset, recursive=True)
    ...
    for item in generator_beheerobjecten:
        eminfra_client.assets.deactiveer_asset(asset=item)
        eminfra_client.assets.activeer_asset(asset=item)
    ...
    eminfra_client.beheerobject.wijzig_boomstructuur(childAsset=childAsset, parentAsset=parentAsset, parentType=BoomstructuurAssetTypeEnum.ASSET)
    eminfra_client.beheerobject.wijzig_boomstructuur(childAsset=childAsset, parentAsset=installatie, parentType=BoomstructuurAssetTypeEnum.BEHEEROBJECT)
    ...
    generator_beheerobjecten = eminfra_client.beheerobject.search_beheerobjecten(naam='JE')