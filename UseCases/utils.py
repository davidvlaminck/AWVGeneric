...
def build_query_search_betrokkenerelaties(bron_asset: AssetDTO = None, agent: AgentDTO = None, rol: str = None)\
        -> QueryDTO:
    if all(par is None for par in (bron_asset, agent, rol)):
        raise ValueError("At least one of the parameters 'bronAsset', 'agent', or 'rol' must be provided.")
    exprs: list[ExpressionDTO] = []
    if bron_asset:
        _append_eq_expr(exprs, "bronAsset", bron_asset.uuid)
    if agent:
...
def build_query_search_assets(bron_asset: AssetDTO = None, agent: AgentDTO = None, actief: bool = True) -> QueryDTO:
    if all(par is None for par in (bron_asset, agent)):
...
def build_betrokkenerelatie(client: EMInfraClient, source: AssetDTO, agent_naam: str, rol: str) -> RelatieObject | None:
    ...
    return create_betrokkenerelation(rol=rol, source_typeURI=source.type.uri, source_uuid=source.uuid,
                                     target_uuid=agent_uuid, target_typeURI=agent_uri)
...
def create_relatie_if_missing(client: EMInfraClient, bron_asset: AssetDTO, doel_asset: AssetDTO,
    ...
    :return:
    """
    logging.info(f'Create relatie {relatie.value} between ({bron_asset.uuid}) and ({doel_asset.uuid}).')
